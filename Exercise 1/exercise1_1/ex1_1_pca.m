...
subplot(1,2,1);
plot(X(:,1),X(:,2),'*');
title('Original dataset - Not normalized');
grid on;
...
%plot the normalized X_norm based on samples
subplot(1,2,2);
plot(X_norm(:,1),X_norm(:,2),'*');
title('Original Dataset - Normalized - Dimension direction');
grid on;
...
title('PCA reconstructed dataset');
